# CX Knowledgebase : Helm-based Deployment for Expertflow CX-Draft
...
TLS/SSL certificate| It is mandatory to have a valid SSL certificate already create in both expertflow and external namespaces. Default Ingress certificate name is “`ef-ingress-tls-secret`"| It is by default required and must be created before the actual helm chart deployment .  
...
CX helm charts are divided into functional groups. Refer to [this document](https://expertflow-docs.atlassian.net/wiki/x/FgDEXQ) for details on their structure.
...
[code] 
    git clone -b <TargetRelease>  https://efcx:RecRpsuH34yqp56YRFUb@gitlab.expertflow.com/cim/cim-solution.git CX-4.10
[/code]
[code] 
    cd <TargetRelease>/kubernetes
[/code]

### Step 2: Create Namespaces

  1. Create a [namespace](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/) `cx` for all cx components
...
[code] 
    kubectl create namespace cx
[/code]

Create a namespace `external` for all the external elements of the Expertflow CX solution such as Mongo, Redis, MinIO, etc.
...
Solution is shipped with self signed certificates. For production deployment we must use the valid commercial certificates.

  
**Adding Custom TLS Certificates**

  * For Commercial Certificates, please import them as `tls.crt` and `tls.key` and create secret with the name of `ef-ingress-tls-secret` in both `external` and `cx` namespaces
...
![](images/icons/grey_arrow_down.png)Custom Password Interpolations (Optional)

The Helm package includes default passwords for MongoDB, MinIO, Redis, PostgreSQL, and ActiveMQ. To change these passwords, update the values in the files listed in the table below.

**Component with a custom password**| **Update required in**  
---|---  
MongoDB| 

  1. Update `CX -> helm-values/values.yaml -> efConnectionVars -> MONGODB_PASSWORD`

  
PostgreSQL| 

  1. Update `keycloak -> helm-values/keycloak-custom-values.yaml -> externalDatabase -> password`
  2. Update `CX -> helm-values/values.yaml -> license-manager -> extraEnvVars-> DB_PASS`

  
minio| 

  1. Update `CX -> helm-values/values.yaml -> file-engin -> extraEnvVars -> ACCESSKEY,SECRETKEY`

  
Redis| 

  1. Update `CX -> helm-values/values.yaml -> efConnectionVars ->`
  2. update `ActiveMQ -> helm-values/active-custom-values.yaml -> extraEnvVars -> REDIS_PASSWORD`
...
You must run the following command before deployment of external components
[code] 
...
[code] 
    kubectl -n external  create configmap ef-postgresql-license-manager-cm --from-file=./pre-deployment/licensemanager/licensemanager.sql
[/code]
...
[code] 
    helm upgrade --install=true --namespace=external --values=helm-values/ef-postgresql-custom-values.yaml  ef-postgresql expertflow/postgresql
[/code]
...
[code] 
    helm upgrade --install=true  --debug --namespace=external  --values=helm-values/ef-keycloak-custom-values.yaml keycloak expertflow/keycloak
[/code]

Check the KeyCloak installation status. You can check the status of deployment by using the following command:
[code] 
    kubectl -n external rollout status sts keycloak
[/code]
...
[code] 
    helm upgrade --install=true --namespace=external --values=helm-values/ef-mongodb-custom-values.yaml mongo expertflow/mongodb
[/code]

Check the MongoDB deployment status by running the following command:
[code] 
    kubectl -n external rollout status sts mongo-mongodb
[/code]
...
[code] 
    helm show values expertflow/minio > helm-values/ef-minio-custom-values.yam
[/code]

Deploy the minio helm chart
[code] 
    helm upgrade --install=true --namespace=external --values=helm-values/ef-minio-custom-values.yaml minio expertflow/minio
[/code]

Wait for the minio deployment to get ready
[code] 
    kubectl -n external  rollout status deployment  minio --timeout=5m
[/code]
...
[/code]
[code] 
    kubectl -n external --timeout=90s wait --for=condition=ready pod minio-helper
[/code]
[code] 
    kubectl -n external cp post-deployment/data/minio/bucket/default minio-helper:/tmp/
[/code]
[code] 
    kubectl -n external cp scripts/icon-helper.sh minio-helper:/tmp/
[/code]
[code] 
    kubectl -n external exec -it minio-helper -- /bin/sh /tmp/icon-helper.sh
[/code]
[code] 
...
[code] 
    helm upgrade --install=true  --namespace=external --values=helm-values/ef-redis-custom-values.yaml  redis expertflow/redis
[/code]
...
[/code]
[code] 
    helm upgrade --install=true  --namespace=external --values=helm-values/ef-activemq-custom-values.yaml activemq expertflow/activemq --version 4.10.0
[/code]
...
[code] 
    helm show values expertflow/clamav --version <Release-Version>   > helm-values/cx-clamav-values.yaml
[/code]
...
[code] 
    helm upgrade --install --namespace external --set global.efCxReleaseName="ef-cx"  clamav --debug --values helm-values/cx-clamav-values.yaml   helm/clamav --version 4.9.0 
[/code]
...
Transfer the Mongo, Redis, PostgreSQL and ActiveMQ Certificates from the external namespace
[code] 
    kubectl get secret mongo-mongodb-ca -n external  -o yaml | sed 's/namespace: external/namespace: expertflow/' | kubectl create -f -
    kubectl get secret redis-crt -n external  -o yaml | sed 's/namespace: external/namespace: expertflow/' | kubectl create -f -
    kubectl get secret ef-postgresql-crt -n external  -o yaml | sed 's/namespace: external/namespace: expertflow/' | kubectl create -f -
    kubectl get secret activemq-tls -n external  -o yaml | sed 's/namespace: external/namespace: expertflow/' | kubectl create -f -
...
[code] 
    helm show values expertflow/cx --version <RELEASE-VERSION>
[/code]
...
“ef-cx” in above command is the release name which will be referenced in all subsequent functional groups' deployments.
...
Create the the database in target Database Management System using the scripts from `CX-<RELESE-VERSION>/kubernetes/pre-deployment/reportingConnector/SQLScripts/dbcreation` directory.
