---
summary: "Entry points for every persona and the reference material you need before you begin."
doc-type: landing
last-updated: 2026-03-13
---

# Getting Started

New to ExpertFlow CX? Start with the [Platform Overview](Expertflow-CX-Platform-Overview.md) for a conceptual introduction to what the platform is and how it fits together.

Then find your role below for a guided first experience.

---

## By Role

| Role | Entry Point |
| --- | --- |
| **Agent (Amy)** | [The Agent Quick-Start Guide](Agent-Quick-Start-Guide.md) |
| **Supervisor (Sam)** | [Monitoring Your Team in Real-Time](Monitoring-Your-Team-in-Real-Time.md) |
| **Solution Admin (Olivia)** | [Unified Admin Guide](Unified-Admin-Guide.md) |
| **Quality Manager (Quentin)** | [Managing the QA Workflow](Managing-the-Quality-Assurance-Workflow.md) |
| **Human Evaluator (Eva)** | [User Guide for Evaluator](Evaluator-Guide.md) |
| **Decision Maker / CTO** | [Decision Maker Guide](../Role_Based_Guides/Decision_Maker/index.md) |
| **Frontend Developer (Dev)** | [AgentManager SDK Reference](../Role_Based_Guides/Frontend_Developer/AgentManager-SDK-Integration-Guide.md) |
| **Conversation Designer (Dave)** | [Conversation Studio Overview](Conversation-Studio-Configuration-Guide.md) |
| **Integration Specialist (Ian)** | [Cisco UCCE/X Integration Reference](Cisco-Contact-Center-Integration-Reference.md) |
| **AI / NLU Specialist** | [Configuring AI-Powered Quality Audits](Configuring-AI-Powered-Quality-Audits.md) |
| **Multi-tenant Partner (Host)** | [Deploying the RKE2 Control Plane](Deploying-the-RKE2-Control-Plane.md) |
| **Reseller (Cloud)** | [Onboarding a New Tenant](Onboarding-a-New-Tenant.md) |

---

## Before You Begin

- [Glossary of Key Terms](Glossary-of-Key-Terms.md) *(coming soon)*
- [What's New — Release Notes](../Role_Based_Guides/Decision_Maker/Release-Lifecycle-and-Versioning.md)
