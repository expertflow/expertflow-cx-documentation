...
summary: "Emitted by Agent Desk when an agent wants to consult with another agent from a queue on the active conversation."
audience: [developer]
product-area: [sdk, platform]
doc-type: reference
difficulty: advanced
keywords: ["consultRequest socket event CX", "consultRequest Agent Manager CX", "Socket.IO CX"]
aliases: ["consultRequest event ExpertFlow", "consultRequest socket CX"]
last-updated: 2026-03-12
---

# consultRequest

Event is emitted when an agent wants to consult on a conversation and sends a consult request to another agent from a certain queue. The Agent Desk emits `consultRequest` and the Routing Engine finds another available agent until the TTL time expires.
...
| `queueName` | String | If mode is `queue`, the name of the queue from which another agent is required. |
