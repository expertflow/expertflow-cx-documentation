---
title: "Platform Operator How-to Guides"
summary: "Task-oriented guides for platform operators — covering deployment, infrastructure, upgrades, and platform health."
audience: [platform-operator]
doc-type: landing
last-updated: 2026-04-07
---

Guides for platform operators responsible for deploying, maintaining, and operating the ExpertFlow CX infrastructure.

## Deployment & Installation

- [Deploying the RKE2 Control Plane](Deploying-the-RKE2-Control-Plane.md)
- [Helm-Based Application Deployment](Helm-Based-Application-Deployment.md)
- [Helm Deployment Customization](Helm-Deployment-Customization.md)
- [Kubernetes Distributions](Kubernetes-Distributions.md)

## Component Deployment

- [CX Voice Deployment](CX-Voice-Deployment.md)
- [CX SIP Proxy Deployment](CX-SIP-Proxy-Deployment-Guide.md)
- [CX SIP Proxy Installation](CX-SIP-Proxy-Installation-Guide.md)
- [CX Dialer Deployment](CX-Dialer-Deployment-Guide.md)
- [CX Analyser Infrastructure Setup](CX-Analyser-Infrastructure-Setup.md)
- [LiveKit Deployment](LiveKit-Deployment-Guide.md)
- [WFM Component Deployment](WFM-Component-Deployment.md)

## Operations & Maintenance

- [Upgrading the Platform](Upgrading-the-Platform.md)
- [Backup and Restore](Backup-and-Restore.md)
- [Monitoring the Platform](Monitoring-the-Platform.md)
- [RKE2 Uninstallation](RKE2-Uninstallation.md)

## Infrastructure Reference

- [Hardware Sizing Calculator](../../Reference/Architecture_and_Infrastructure/Hardware-Sizing-Calculator.md)

---

New to ExpertFlow CX? Start here:

→ [Installation Guide](../../Getting_Started/For_Platform_Operators/Installation-Guide.md)
→ [Upgrade Guide](../../Getting_Started/For_Platform_Operators/Upgrade-Guide.md)
