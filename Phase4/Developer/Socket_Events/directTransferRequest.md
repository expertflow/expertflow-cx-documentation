...
summary: "Emitted by Agent Desk when an agent wants to leave the conversation and transfer it directly to another agent from a queue."
audience: [developer]
product-area: [sdk, platform]
doc-type: reference
difficulty: advanced
keywords: ["directTransferRequest socket event CX", "directTransferRequest Agent Manager CX", "Socket.IO CX"]
aliases: ["directTransferRequest event ExpertFlow", "directTransferRequest socket CX"]
last-updated: 2026-03-12
---

# directTransferRequest

Event is emitted when an agent wants to leave the conversation and transfer it to another agent from a certain queue. The Agent Desk emits `directTransferRequest` and the Routing Engine finds another available agent until the TTL time expires.
...
| `mode` | String | Mode could be `queue` or `agent`. |
| `queueName` | String | If mode is `queue`, the name of the queue from which another agent is required to take over the active conversation. |
...
- [directConferenceRequest](./directConferenceRequest_2531711.md)
