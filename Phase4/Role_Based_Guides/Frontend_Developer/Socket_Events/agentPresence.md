---
title: "agentPresence_"
summary: "Emitted by Agent Manager to update agent state information, including agent MRD states and whether a state change occurred."
audience: [developer]
product-area: [sdk, platform]
doc-type: reference
difficulty: advanced
keywords: ["agentPresence_ socket event CX", "agentPresence_ Agent Manager CX", "Socket.IO CX"]
aliases: ["agentPresence event ExpertFlow", "agentPresence socket CX"]
last-updated: 2026-03-12
---

# agentPresence_

Event triggered to update agent state information. The `action` field indicates whether the event carries a state change or not.
...
- `AGENT_STATE_UNCHANGED` — event received without any change in state.
- `AGENT_STATE_CHANGED` — event received with a change in state.

## Payload Properties

| Property | Type | Description |
|---|---|---|
| `agentPresence` | Object | The AgentPresence object containing full agent state details. |
| `agent` | Object | Defines the properties of the CC agent. Includes: `participantType` (String), `id` (String), `keycloakUser` (Object with agent auth properties), `associatedRoutingAttributes` (Array). |
| `state` | Object | State of the agent (`READY`, `NOT_READY`) and the reason. Includes: `name` (String), `reasonCode` (String). |
| `stateChangeTime` | Date | Epoch timestamp of the last state change. |
| `agentMrdStates` | Object | Retains properties of state changes per MRD. Each entry contains MRD details: `id`, `name`, `description`, `interruptible`, `maxRequests`. |
| `action` | Object | Describes if the event is fired with or without a state change (`AGENT_STATE_CHANGED` or `AGENT_STATE_UNCHANGED`). |
| `agentLoginTime` | Timestamp | Epoch timestamp of agent login. |
| `agentStateChanged` | Boolean | `true` if agent state changed; `false` if only agent MRD state changed. |
| `mrdStateChanges` | Array | IDs of the MRDs whose state is being changed. |
