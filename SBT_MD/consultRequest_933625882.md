# CX Knowledgebase : consultRequest

**Event Name**|  consultRequest  
---|---  
**Event Description**|  Event is emitted when an agent wants to consult in the conversation and send conult req to another agent (from a certain queue) to the active conversation; the agent desk emits an event of '`consultRequest'`. In response, the Routing Engine will find another available agent until TTL time expires.  
**Emitter**|  Agent Desk  
...
offerToAgent| type: boolean if mode is agent then it will be true|   
